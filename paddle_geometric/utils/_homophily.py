...
def homophily(edge_index: Adj, y: paddle.Tensor, batch: OptTensor = None,
              method: str = "edge") -> Union[float, paddle.Tensor]:
    ...
    if isinstance(edge_index, SparseTensor):
        ...
        row, col = edge_index
    if method == "edge":
        out = paddle.zeros(shape=row.shape[0])
        out[y[row] == y[col]] = 1.0
        if batch is None:
            return float(out.mean())
        else:
            dim_size = int(batch._max()) + 1
        ...
        out = (h - proportions).clip_(min=0).sum(axis=-1)
        out /= num_classes - 1
