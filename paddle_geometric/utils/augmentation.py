...
def shuffle_node(x: paddle.Tensor, batch: Optional[paddle.Tensor] = None,
                 training: bool = True) -> Tuple[paddle.Tensor, paddle.Tensor]:
    ...
    """
    if not training:
        perm = paddle.arange(end=x.shape[0])
        return x, perm
    if batch is None:
        perm = paddle.randperm(n=x.shape[0])
        return x[perm], perm
    num_nodes = scatter(paddle.ones(shape=x.shape[0], dtype=batch.dtype),
                        batch, dim=0, reduce="sum")
    ptr = cumsum(num_nodes)
    perm = paddle.concat(x=[(paddle.randperm(n=n) + offset)
...
def mask_feature(
    ...
    if not training or p == 0.0:
        return x, paddle.ones_like(x=x, dtype="bool")
    assert mode in ["row", "col", "all"]
    if mode == "row":
        mask = paddle.rand(shape=x.shape[0]) >= p
        mask = mask.view(-1, 1)
    elif mode == "col":
        mask = paddle.rand(shape=x.shape[1]) >= p
        mask = mask.view(1, -1)
    else:
        mask = paddle.rand(shape=x.shape, dtype=x.dtype) >= p
    x = x.masked_fill(mask=~mask, value=fill_value)
    return x, mask
...
def add_random_edge(
    ...
    if p < 0.0 or p > 1.0:
        raise ValueError(
            f"Ratio of added edges has to be between 0 and 1 (got '{p}')")
    ...
    if not training or p == 0.0:
        edge_index_to_add = paddle.tensor([[], []], device=device)
    ...
    edge_index = paddle.concat(
        x=[edge_index, edge_index_to_add.cast(edge_index.dtype)], axis=1)
    return edge_index, edge_index_to_add
