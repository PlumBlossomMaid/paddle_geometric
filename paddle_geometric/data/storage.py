...
from paddle_geometric.data.view import ItemsView, KeysView, ValuesView
    ...
    def concat(self, other: Self) -> Self:
        ...
        for key, dim in self._cat_dims(self.keys()).items():
            ...
            if key in {'num_nodes', 'num_edges'}:
                ...
                self[key] = paddle.concat([value1, value2], axis=dim)
    ...
    * :obj:`adj_t`: A **transposed** :class:`torch_sparse.SparseTensor` holding
    ...
    def num_edges(self) -> int:
        ...
        for value in self.values('adj', 'adj_t'):
            if isinstance(value, SparseTensor):
                return value.nnz()
            elif is_paddle_sparse_tensor(value):
                return value._nnz()
        return 0
    ...
    def is_sorted(self, sort_by_row: bool = True) -> bool:
        if 'edge_index' in self:
            ...
            return bool(paddle.all(index[:-1] <= index[1:]))
        return True
    ...
    def has_isolated_nodes(self) -> bool:
        ...
        if self.is_bipartite():
            return paddle.unique(edge_index[1]).numel() < num_nodes
        else:
