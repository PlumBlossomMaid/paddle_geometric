    ...
    * To initialize an edge from source node type :obj:`"author"` to
    ...
    def subgraph(self, subset_dict: Dict[NodeType, Tensor]) -> Self:
        ...
        for edge_type in self.edge_types:
            ...
            if src_subset is None:
                src_subset = paddle.arange(data[src].num_nodes)
            dst_subset = subset_dict.get(dst)
            if dst_subset is None:
                dst_subset = paddle.arange(data[dst].num_nodes)
    ...
    def to_homogeneous(
        ...
        if 'edge_label_index' in self:
            ...
            data.edge_label_index = paddle.concat(
                list(edge_label_index_dict.values()), axis=-1)
    ...
    def _put_edge_index(self, edge_index: EdgeTensorType,
        ...
        if edge_attr.layout == EdgeLayout.COO:
            store.edge_index = paddle.stack([row, col], axis=0)
