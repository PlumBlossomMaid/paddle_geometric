...
import paddle
import pytest
from paddle.nn import Linear as PTLinear

import paddle_geometric.backend
from paddle_geometric import place2devicestr
from paddle_geometric.nn import HeteroDictLinear, HeteroLinear, Linear
from paddle_geometric.nn.dense.linear import (
...
# from paddle.nn.parameter import UninitializedParameter
...
def test_linear(weight, bias, device):
    x = paddle.randn(3, 4, 16, device=device)
...
def test_lazy_linear(weight, bias, device):
    ...
    assert place2devicestr(lin.weight.place) == device
    assert place2devicestr(lin.bias.place) == device
    assert str(lin) == 'Linear(-1, 32, bias=True)'
    assert lin(x).size() == (3, 4, 32)
    assert str(lin) == 'Linear(16, 32, bias=True)'

    assert place2devicestr(copied_lin.weight.place) == device
    assert place2devicestr(copied_lin.bias.place) == device
...
def test_load_lazy_linear(dim1, dim2, bias, device):
    ...
    if dim1 != -1:
        assert isinstance(
            lin1.weight,
            (paddle.nn.Parameter, paddle.base.framework.EagerParamBase))
        assert isinstance(
            lin2.weight,
            (paddle.nn.Parameter, paddle.base.framework.EagerParamBase))
        assert paddle.allclose(lin1.weight, lin2.weight)
        assert not hasattr(lin1, '_hook')
        assert not hasattr(lin2, '_hook')
    else:
        assert is_uninitialized_parameter(lin1.weight)
        assert is_uninitialized_parameter(lin2.weight)
    ...
    if bias:
        ...
        if dim1 != -1:  # Only check for equality on materialized bias:
            assert paddle.allclose(lin1.bias, lin2.bias)
    else:
...
def test_identical_linear_default_initialization(lazy):
    ...
    lin2 = PTLinear(16, 32)
    paddle.manual_seed(12345)
    reset_weight_(lin2.weight, in_channels=16)
    reset_bias_(lin2.bias, in_channels=16)

    assert paddle.equal_all(lin1.weight, lin2.weight).item()
    assert paddle.equal_all(lin1.bias, lin2.bias).item()
    assert paddle.allclose(lin1(x), lin2(x))


def test_copy_unintialized_parameter():
    weight = paddle.base.framework.EagerParamBase.from_tensor(
...
def test_copy_linear(lazy, device):
    ...
    if not is_uninitialized_parameter(copied_lin.weight):
    ...
    if not is_uninitialized_parameter(copied_lin.weight):
        assert copied_lin.weight.data_ptr() != lin.weight.data_ptr()
        assert paddle.allclose(copied_lin.weight, lin.weight)
    assert id(copied_lin.bias) != id(lin.bias)
    assert copied_lin.bias.data_ptr() != lin.bias.data_ptr()
    if int(paddle.isnan(lin.bias).sum()) == 0:
        assert paddle.allclose(copied_lin.bias, lin.bias)


@withCUDA
def test_hetero_linear_basic(device):
    x = paddle.randn(3, 16, device=device)
    type_vec = paddle.to_tensor([0, 1, 2], place=device)
    ...
    # jit = paddle.jit.script(lin)
    # assert paddle.allclose(jit(x, type_vec), out, atol=1e-3)
...
def test_hetero_linear_initializer():
    ...
    assert paddle.equal_all(lin.bias, paddle.zeros_like(lin.bias)).item()


@withCUDA
# @pytest.mark.parametrize('use_segment_matmul', [None, True, False])
@pytest.mark.parametrize('use_segment_matmul', [None, False])
def test_hetero_linear_amp(device, use_segment_matmul):
    warnings.filterwarnings('ignore', '.*but CUDA is not available.*')

    old_state = paddle_geometric.backend.use_segment_matmul
    paddle_geometric.backend.use_segment_matmul = use_segment_matmul
    ...
    with paddle.amp.auto_cast('cuda'):
        assert lin(x, type_vec).size() == (3, 32)

    paddle_geometric.backend.use_segment_matmul = old_state
...
def test_hetero_dict_linear(bias, device):
    x_dict = {
        'v': paddle.randn(3, 16, device=device),
        'w': paddle.randn(2, 8, device=device),
    }
    ...
    x_dict = {
        'v': paddle.randn(3, 16, device=device),
        'w': paddle.randn(2, 16, device=device),
    }
...
@pytest.mark.parametrize('type_vec', [
    paddle.to_tensor([0, 0, 1, 1, 2, 2]),
    paddle.to_tensor([0, 1, 2, 0, 1, 2]),
])
def test_hetero_linear_sort(type_vec, device):
    x = paddle.randn(type_vec.numel(), 16, device=device)
    ...
    for i in range(type_vec.numel()):
        ...
        assert paddle.allclose(out[i], expected, atol=1e-3)
...
#         num_nodes_list = paddle.normal(
#             mean=paddle.tensor([mean] * num_types, dtype=paddle.float),
#             std=paddle.tensor([std] * num_types, dtype=paddle.float),
#         ).round().to(paddle.long).tolist()

#         return [
#             paddle.randn(num_nodes, channels, device=args.device)
...
#         x = paddle.nested.nested_tensor(xs)
#         weight = paddle.nested.nested_tensor(weights)
#         return list(paddle.matmul(x, weight).unbind(0))
...
#         count = paddle.tensor([x.size(0) for x in xs])
#         ptr = cumsum(paddle.tensor([x.size(0) for x in xs]))
#         x = paddle.cat(xs, dim=0)
#         padded_x = paddle.nested.nested_tensor(xs).to_padded_tensor(padding=0.0)
#         weight = paddle.randn(num_types, channels, channels, device=args.device)
