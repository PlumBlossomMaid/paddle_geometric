...
def test_in_memory_dataset():
    x1 = paddle.to_tensor([[1.0], [1.0], [1.0]])
    x2 = paddle.to_tensor([[2.0], [2.0], [2.0]])
    edge_index = paddle.to_tensor([[0, 1, 1, 2], [1, 0, 2, 1]])
    face = paddle.to_tensor([[0], [1], [2]])
    ...
    assert paddle.equal_all(dataset.x, paddle.cat([x1, x2], dim=0)).item()
    assert dataset.edge_index.tolist() == [
        [0, 1, 1, 2, 10, 11, 11, 12],
        [1, 0, 2, 1, 11, 10, 12, 11],
    ]
    assert paddle.equal_all(dataset[1:].x, x2).item()


# def test_stored_in_memory_dataset(tmp_path):
#     x1 = paddle.to_tensor([[1.0], [1.0], [1.0]])
#     x2 = paddle.to_tensor([[2.0], [2.0], [2.0], [2.0]])
#     edge_index = paddle.to_tensor([[0, 1, 1, 2], [1, 0, 2, 1]])
...
# assert paddle.equal_all(dataset[0].x, x1).item()
# assert paddle.equal_all(dataset[0].edge_index, edge_index).item()
# assert dataset[0].num_nodes == 3
# assert paddle.equal_all(dataset[0].test_int, paddle.to_tensor([1])).item()
# assert dataset[0].test_str == '1'

# assert paddle.equal_all(dataset[1].x, x2).item()
# assert paddle.equal_all(dataset[1].edge_index, edge_index).item()
# assert dataset[1].num_nodes == 4
# assert paddle.equal_all(dataset[1].test_int, paddle.to_tensor([2])).item()
# assert dataset[1].test_str == '2'

# def test_stored_hetero_in_memory_dataset(tmp_path):
#     x1 = paddle.tensor([[1.0], [1.0], [1.0]])
#     x2 = paddle.tensor([[2.0], [2.0], [2.0], [2.0]])
...
#     assert paddle.equal(dataset[0]['paper'].x, x1)
#     assert dataset[0]['paper'].num_nodes == 3

#     assert paddle.equal(dataset[1]['paper'].x, x2)
...
def test_index(tmp_path):
    ...
    for data, index in zip(dataset, [index1, index2]):
        ...
        assert data.batch.equal_all(index).item()
...
def test_edge_index(tmp_path):
    ...
    for data, edge_index in zip(dataset, [edge_index1, edge_index2]):
        ...
        assert data.edge_index.equal_all(edge_index).item()
...
def test_in_memory_num_classes():
    ...
    dataset = MyTestDataset([
        Data(y=paddle.tensor([[0, 1, 0, 1]])),
        Data(y=paddle.tensor([[1, 0, 0, 0]])),
        Data(y=paddle.tensor([[0, 0, 1, 0]])),
    ])
...
def test_in_memory_dataset_copy():
    data_list = [Data(x=paddle.randn(5, 16)) for _ in range(4)]
    ...
    for copied_data, data in zip(copied_dataset, dataset):
        assert paddle.equal_all(copied_data.x, data.x).item()

    copied_dataset = dataset.copy([1, 2])
    assert len(copied_dataset) == 2
    assert paddle.equal_all(copied_dataset[0].x, data_list[1].x).item()
    assert paddle.equal_all(copied_dataset[1].x, data_list[2].x).item()


# def test_to_datapipe():
#     x = paddle.randn(3, 8)
#     edge_index = paddle.tensor([[0, 1, 1, 2], [1, 0, 2, 1]])
#     data = Data(x=x, edge_index=edge_index)
#     dataset = MyTestDataset([data, data])

#     dp = dataset.to_datapipe()

#     assert isinstance(dp, paddle.utils.data.IterDataPipe)
#     assert len(dp) == 2

#     assert paddle.equal(dataset[0].x, list(dp)[0].x)
#     assert paddle.equal(dataset[0].edge_index, list(dp)[0].edge_index)
#     assert paddle.equal(dataset[1].x, list(dp)[1].x)
#     assert paddle.equal(dataset[1].edge_index, list(dp)[1].edge_index)


@withPackage('paddle_sparse')
def test_in_memory_sparse_tensor_dataset():
    x = paddle.randn(11, 16)
    adj = SparseTensor(
        row=paddle.tensor([4, 1, 3, 2, 2, 3]),
        col=paddle.tensor([1, 3, 2, 3, 3, 2]),
        sparse_sizes=(11, 11),
    )
    ...
    dataset = MyTestDataset([data, data])
    assert len(dataset) == 2
    assert paddle.allclose(dataset[0].x, x)
    assert dataset[0].adj.sparse_sizes() == (11, 11)
    assert paddle.allclose(dataset[1].x, x)
...
def test_lists_of_tensors_in_memory_dataset():
    ...
    assert tuple(dataset[0].xs[1].shape) == (11, 4)
    assert tuple(dataset[0].xs[2].shape) == (1, 2)
    assert tuple(dataset[1].xs[0].shape) == (5, 3)
    assert tuple(dataset[2].xs[1].shape) == (15, 4)
    assert tuple(dataset[3].xs[1].shape) == (16, 4)


@withPackage('paddle_sparse')
def test_lists_of_sparse_tensors():
    e1 = paddle.to_tensor([[4, 1, 3, 2, 2, 3], [1, 3, 2, 3, 3, 2]])
    e2 = paddle.to_tensor([[0, 1, 4, 7, 2, 9], [7, 2, 2, 1, 4, 7]])
    e3 = paddle.to_tensor([[3, 5, 1, 2, 3, 3], [5, 0, 2, 1, 3, 7]])
    e4 = paddle.to_tensor([[0, 1, 9, 2, 0, 3], [1, 1, 2, 1, 3, 2]])
...
#         'x': dict(dtype=paddle.float32, size=(-1, 34)),
#         'edge_index': dict(dtype=paddle.int64, size=(2, -1)),
#         'y': dict(dtype=paddle.int64, size=(-1, )),
#         'train_mask': dict(dtype=paddle.bool, size=(-1, )),
#     }
...
#     assert paddle.equal(data1.edge_index, data2.edge_index)
#     assert paddle.equal(data1.y, data2.y)
#     assert paddle.equal(data1.train_mask, data2.train_mask)
