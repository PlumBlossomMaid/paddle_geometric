...
from paddle_geometric.data import Data, Dataset, InMemoryDataset
...
class MyData(Data):
    ...
    def random(self):
        return paddle.randn(list(self.x.shape) + list(self.arg.shape))
...
def test_inherit():
    ...
    data = dataset[0]
    assert tuple(data.random().shape) == (4, 5, 4, 3)
