...
from paddle_geometric.data.storage import BaseStorage


def test_base_storage():
    storage = BaseStorage()
    assert storage._mapping == {}
    storage.x = paddle.zeros([
        1,
    ])
    storage.y = paddle.ones([
        1,
    ])
    ...
    assert paddle.allclose(storage.get('x', None), storage.x)
    assert paddle.allclose(storage.get('y', None), storage.y)
    ...
    storage = BaseStorage({'x': paddle.zeros([
        1,
    ])})
    assert len(storage) == 1
    assert storage.x is not None
...
def test_storage_tensor_methods():
    ...
    assert place2devicestr(storage.x.place) == 'cpu'
